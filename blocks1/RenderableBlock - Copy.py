...
from ctypes import byref, cdll, c_void_p, c_double, c_long, create_string_buffer, POINTER,memmove,addressof
import sip,math

from Block import Block
from BlockShape import BlockShape
from InfixBlockShape import InfixBlockShape
from CollapseLabel import CollapseLabel
from ConnectorTag import ConnectorTag
from NameLabel import NameLabel
from BlockLabel import BlockLabel
from PageLabel import PageLabel
from SocketLabel import SocketLabel
...
from BlockLinkChecker import BlockLinkChecker
...
class RenderableBlock(QtGui.QWidget):
   ...
   def __init__(self,workspaceWidget, blockID, isLoading=False,back_color=QtGui.QColor(225,225,225,255)):
      ...
      self.blockLabel = NameLabel(self.getBlock().getBlockLabel(), BlockLabel.Type.NAME_LABEL, self.getBlock().isLabelEditable, self.blockID);
   ...
   def getRenderableBlock(blockID):
      if(blockID in RenderableBlock.ALL_RENDERABLE_BLOCKS):
         ...
         print("Can not find block" + blockID)
   ...
   def moveConnectedBlocks(self):
      ...
      for socket in BlockLinkChecker.getSocketEquivalents(b):
         ...
         if (socket.hasBlock()) :
            ...
            if (rb == None):
               print("Block doesn't exist yet: "+socket.getBlockID());
               continue;
   ...
   def enterEvent(self,event):
      ...
      painter = QtGui.QPainter(self.buffImg);
      pen = QtGui.QPen()
   ...
   def drawHighlightSocket(self,link,hightlight):
      if(link != None):
         ...
         if(hightlight):
            painter = QtGui.QPainter(peer_rb.buffImg);
            pen = QtGui.QPen()
            pen.setColor(QtGui.QColor(255,170,0,255))
            pen.setWidth(3)
            painter.setPen(pen);
